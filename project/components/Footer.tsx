...
export default function Footer() {
  ...
  const copyEmail = async () => {
    await navigator.clipboard.writeText('siaoivan@gmail.com');
    setCopied(true);
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-10 mb-10">
          ...
          <div>
            ...
            <p className="text-foreground/40 text-sm leading-relaxed mb-4">
              AI Automation Specialist building intelligent workflows that save
            ...
            <div className="mt-3 flex items-center gap-2 text-sm">
              ...
              <button
                onClick={copyEmail}
                className="ml-2 inline-flex items-center gap-1 rounded-lg border border-foreground/10 px-2 py-1 text-xs text-foreground/60 hover:border-[#4F46E5] hover:text-white transition-all"
              >
                {copied ? (
                  <>
          ...
          {/* Navigation */}
          <div>
            <h4 className="text-foreground/70 text-sm font-semibold uppercase tracking-wide mb-4">
              Navigation
            </h4>
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
          <p className="text-foreground/30 text-sm">
            &copy; {new Date().getFullYear()} Ivan Xavier Siao. All rights
            reserved.
          </p>
